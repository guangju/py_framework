...
class DouyinAuthorDetailProvider(urlprovider.UrlProvider):
    """
    classdocs
    """

    url = "http://api.amemv.com/aweme/v1/user/?user_id={user_id}&aweme_retry_count=0&iid={iid}&device_id={device_id}&ac=wifi&channel=vivo&aid=1128&app_name=aweme&version_code=156&version_name=1.5.6&device_platform=android&ssmix=a&device_type=vivo+Y67A&device_brand=vivo&os_api=23&os_version=6.0&uuid={uuid}&openudid={openudid}&manifest_version_code=156&resolution=720*1280&dpi=320&update_version_code=1561&app_type=normal"
    ...
    def onReceiveMsg(self, msg):
        ...
        :param msg:
        """
        log.debug("DouyinAuthorDetailProvider receive {}".format(msg))
    ...
    def next(self):
        ...
        log.debug("DouyinAuthorDetailProvider next: {}".format(urlPack))
        return urlPack
