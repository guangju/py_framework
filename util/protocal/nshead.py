...
class nshead:
    ...
    def load(self, bindata):
        (self.head["id"], self.head["version"], self.head["log_id"], self.head["provider"], self.head["magic_num"],self.head["reserved"], self.head["body_len"]) = self.struct.unpack(bindata)
